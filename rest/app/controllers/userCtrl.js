...
exports.forgotPassword = (req,res,next) => {
    ...
    .then(([userData,userMeta])=> {
        ...
        if(userData.length===2){
            ...
            if(validate===-2){
                return res.json(returnResJsonObj.resJsonOjbect(true,`Nao foi encontrado nenhum usuario ativo para este e-mail!`, validate))   }
        }
        var data = userData[0]
        if(data){
            ...
            .then(([data2,meta2])=>{
                if(data2.affectedRows>0){
                    ...
                    return res.json(returnResJsonObj.resJsonOjbect(false,`E-mail para recuperacao de senha enviado para ${user.email}`,noError))            
                }else{
                    return res.json(returnResJsonObj.resJsonOjbect(true,'Erro no servidor. Nao foi possivel criar token para recuperacao de senha.',userError))}})
            .catch(err => {
                console.log(err)
                return res.json(returnResJsonObj.resJsonOjbect(true,'Erro no servidor. Nao foi possivel criar token para recuperacao de senha.',userError))})
        }else{
            return res.json(returnResJsonObj.resJsonOjbect(true,'Nao foi encontrado nenhum usuario ativo para este e-mail!',userError))}})
    .catch(err => {
        console.log(err)
        return res.json(returnResJsonObj.resJsonOjbect(true,'Nao foi encontrado nenhum usuario ativo para este e-mail!',userError))})
}

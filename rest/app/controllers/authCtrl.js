...
exports.register = (req,res,next)=> {
    ...
    if(!req.body.email||!req.body.password||!req.body.confirmPassword||!req.body.name||!req.body.phoneNumber||!req.body.facebookName||!req.body.instagramName){
        console.log('returnCalled')
        //code: 'string', removed
        return res.json(returnResJsonObj.resJsonOjbect(true,`{invalidFormat: true,neededFields: {email: 'string',password: 'string',confirmPassword: 'string',phoneNumber:'string',name:'string',}}`,authError))  }
    ...
    if(!validator.validateEmailAddress(email)){
        ...
        validationObject.msg = validationObject.msg + 'Voce deve informar um e-mail valido. '    }
    //validating passwords
    //check if passowrds match
    if(!passwordValidation.checkPasswordAndPasswordConfirmationEquality(password,confirmPassword)){
        validationObject.hasErrors = true
        validationObject.msg = validationObject.msg + 'Senha e confirmacao de senha devem ser iguais. '   }
    if(!passwordValidation.checkPasswordAndPasswordConfirmationLength(password,confirmPassword)){
        validationObject.hasErrors = true
        validationObject.msg = validationObject.msg + 'Senha deve ter no minimo 3 e no maximo 20 caracteres. '   }
    //validatePassword
    if(!passwordValidation.validatePasswords(password,confirmPassword)){
        validationObject.hasErrors = true
        validationObject.msg = validationObject.msg + 'Senha deve ter no minimo 3 e no maximo 20 caracteres e deve conter no minimo uma letra maiuscula, uma letra minuscula e um numero.'  }
...
exports.login = (req,res,next)=>{
    ...
    .then(([data,meat])=> {
        console.log(data)
        if(data.length<=2){
            if(data[1][0].returnCode===-2){
                return res.json(returnResJsonObj.resJsonOjbect(true,`Nao foi encontrado nenhum usuario ativo para este e-mail!`,authError))    
            }
            return res.json(returnResJsonObj.resJsonOjbect(true,`E-mail ja esta sendo usado por um usuario na nossa base de dados!`,authError))    }
        ...
        .then(correct => {
            ...
            if(correct){
                ...
                return res.json(returnResJsonObj.resJsonOjbect(true,`Senha incorreta.`,authError))}})
        .catch(err => {
